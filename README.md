# Social network for trips - System Design

System Design социальной сети для путешествий для курса по System Design – https://balun.courses/courses/system_design

### Functional requirements:

- публикация поста с текстом, картинками и местом путешествия
- места путешествий преподготовленные
- оценка поста
- комментарий под постом
- подписка на путешественников
- поиск популярных (с наивысшим средним рейтинг) мест для путешествий
- просмотр постов по популярным местам для путешествий
- просмотр ленты других путешественников и ленты пользователя, основанной на подписках в обратном хронологическом порядке
- просмотр комментариев под постом

### Non-functional requirements:

- 10 000 000 DAU
- доступность 99,9%
- посты храним 20 лет, пользователей всегда
- клиенты - мобильные устройства и браузер
- гео распределение по странам СНГ
- сезонность есть - праздники, лето
Активность пользователей
- пользователь в среднем просматривает 3 ленты (1 лента = 5 постов) в день в несезон и 9 лент в день в сезон
- пользователь в среднем постит раз в месяц в несезон и раз в неделю в сезон
- пользователь в среднем читает 30 комментариев в несезон и 90 комментариев в сезон
- пользователь в среднем пишет комментарий раз в неделю в несезон и раз в день в сезон
- пользователь в среднем ставит оценку 3 раза в день в несезон и 9 раз в день в сезон
- пользователь просматривает ленту популярных мест для путешествий 1 раз в день в несезон и 3 раза в день в сезон
Ограничения:
- max количество картинок в посте - 10
- max размер изображения - 500 КБ
- max размер текста в посте - 2000 символов
- max размер комментария - 500 символов
Тайминги:
- получение ленты - 1 секунда
- просмотр комментариев - 1 секунда
- создание поста - 5 секунд

## Basic calculations

### Создание поста (write)

#### RPS

    DAU = 10 000 000
    1 пост в неделю в сезон
    в сезон RPS = 10 000 000 / 86 400 / 7 ~= 20 rps 
    1 пост в месяц в несезон
    в несезон RPS = 10 000 000 / 86 400 / 30 ~= 4 rps

#### Transaction

Post

    post_id ~ 16 bytes
    user_id ~ 16 bytes
    text ~ 2000 * 2 bytes
    imgs ~ 10 * 500 kb
    place_id ~ 16 bytes

Итого:

    meta ~ 48 bytes
    text ~ 4kb
    static ~ 5mb

#### Трафик

    traffic в сезон = 20 * 5mb = 100 mb/s
    traffic в несезон = 4 * 5mb = 20 mb/s

### Получение ленты (read)

#### RPS

    DAU = 10 000 000
    9 лент в день в сезон
    в сезон RPS = 10 000 000 / 86400 * 9 ~= 900 rps
    3 ленты в день в несезон
    в несезон RPS = 10 000 000 / 86400 * 3 ~= 300 rps

#### Transaction

    post = post при создании + reactions_count (8 bytes)
    Лента = 5 * post ~= 5 * 5mb ~= 25mb

#### Трафик

    traffic в сезон = 900 * 25mb = 22 gb/s
    traffic в несезон = 300 * 25mb = 7 gb/s

### Получение ленты популярных мест (read)

#### RPS

    DAU = 10 000 000
    3 лент в день в сезон
    в сезон RPS = 10 000 000 / 86400 * 3 ~= 300 rps
    1 ленты в день в несезон
    в несезон RPS = 10 000 000 / 86400 * 1 ~= 100 rps

#### Трафик

    traffic в сезон = 300 * 25mb = 7 gb/s
    traffic в несезон = 100 * 25mb = 3 gb/s

### Написание комментариев (write)

#### RPS

    DAU = 10 000 000
    1 раз в день в сезон
    в сезон RPS = 10 000 000 / 86400 ~= 100 rps
    1 раз в неделю в несезон
    в несезон RPS = 10 000 000 / 86400 / 7 ~= 20 rps

#### Трафик

    traffic в сезон = 100 * 500 = 50 kb/s
    traffic в несезон = 20 * 500 = 10 kb/s

### Просмотр комментариев (read)

#### RPS

    DAU = 10 000 000
    90 раз в день в сезон
    в сезон RPS = 10 000 000 / 86400 * 90 ~= 9k rps
    30 раз в день в несезон
    в несезон RPS = 10 000 000 / 86400 * 30 ~= 3k rps

#### Трафик

    traffic в сезон = 9000 * 500 = 5 mb/s
    traffic в несезон = 3000 * 500 = 2 mb/s

### оценивание/реакции (write)

#### RPS

    DAU = 10 000 000
    9 раз в день в сезон
    в сезон RPS = 10 000 000 / 86400 * 9 ~= 900 rps
    3 раз в неделю в несезон
    в несезон RPS = 10 000 000 / 86400 * 3 ~= 300 rps

#### Transaction

    post_id ~ 16 bytes
    point ~ 8 bytes

Итого 24 bytes


#### Трафик

    traffic в сезон = 900 * 24 = 23 kb/s
    traffic в несезон = 300 * 24 = 8 kb/s
